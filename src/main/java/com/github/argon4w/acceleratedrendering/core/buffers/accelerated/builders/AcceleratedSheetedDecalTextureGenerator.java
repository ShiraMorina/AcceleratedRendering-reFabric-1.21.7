...
public class AcceleratedSheetedDecalTextureGenerator extends AcceleratedVertexConsumerWrapper {
    ...
    public static Direction getNearest(float x, float y, float z) {
        ...
        for (Direction direction2 : Direction.values()) {
            Vec3i normal = new Vec3i(direction2.getStepX(), direction2.getStepY(), direction2.getStepZ());
            float g = x * (float) normal.getX() + y * (float) normal.getY() + z * (float) normal.getZ();
            if (g > f) {
                f = g;
    ...
    @Override
    public @NotNull VertexConsumer setNormal(
            float pNormalX,
            float pNormalY,
            float pNormalZ
    ) {
